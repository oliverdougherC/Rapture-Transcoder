...
def load_config():
    ...
    try:
        ...
        # Expand user paths and create directories if they don't exist
        for key in ['input_directory', 'output_directory']:
            if key in config:
                config[key] = os.path.expanduser(config[key])
                logger.debug(f"Expanded path for {key}: {config[key]}")
                try:
                    if not os.path.exists(config[key]):
                        os.makedirs(config[key])
                        logger.info(f"Created directory: {config[key]}")
                    else:
                        logger.debug(f"Directory already exists: {config[key]}")
                except Exception as e:
                    logger.error(f"Error creating directory {config[key]}: {str(e)}")
        
        # Only process movie and TV directories if media detection is enabled
        if config['use_media_detection']:
            for key in ['movie_output_directory', 'tv_output_directory']:
                if key in config:
                    config[key] = os.path.expanduser(config[key])
                    logger.debug(f"Expanded path for {key}: {config[key]}")
        else:
            # Remove movie and TV directories from config if media detection is disabled
            config.pop('movie_output_directory', None)
            config.pop('tv_output_directory', None)
...
def detect_gpu():
    try:
        ...
        # Check for Intel GPU (integrated graphics)
        system_profiler = subprocess.run(["system_profiler", "SPDisplaysDataType"], capture_output=True, text=True)
        if "Intel" in system_profiler.stdout:
            return "intel"
        
        # Check for dedicated AMD GPU
        if "AMD" in system_profiler.stdout:
            return "amd"
        
        # Check for dedicated NVIDIA GPU
        if "NVIDIA" in system_profiler.stdout:
            return "nvidia"
    except Exception as e:
...
def process_directory(config):
    ...
    if use_media_detection:
        movie_dir = config.get('movie_output_directory')
        tv_dir = config.get('tv_output_directory')
        if movie_dir:
            dirs_to_check.append(("Movie", movie_dir))
        if tv_dir:
            dirs_to_check.append(("TV", tv_dir))
    ...
    for dir_name, dir_path in dirs_to_check:
        try:
            if not os.path.exists(dir_path):
                ...
                logger.debug(f"{dir_name} directory already exists: {dir_path}")
        except Exception as e:
            logger.error(f"Error creating {dir_name} directory {dir_path}: {str(e)}")
